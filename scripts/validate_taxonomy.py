...
t = yaml.safe_load(open(os.path.join(ROOT, "config", "taxonomy.yaml")))
errors, warnings = [], []
...
for combo in r["exactly_one_of"]:
    for s in combo:
        if s not in slot_ids: errors.append(f"exactly_one_of unknown slot {s}")
for s in r["exactly_one"] + r["at_most_one"]:
    if s not in slot_ids: errors.append(f"unknown slot {s}")
for rng in r["ranges"]:
    if rng["slot"] not in slot_ids: errors.append(f"range unknown slot {rng['slot']}")
    if rng["min"] > rng["max"]: errors.append(f"range {rng['slot']}: min > max")
...
for pair in r["forbidden_pairs"]:
    for s in pair:
        if s not in flat: errors.append(f"forbidden_pairs unknown subcategory {s}")
for ov in t["slot_overrides"]:
    if ov["subcategory"] not in flat: errors.append(f"slot_override unknown sub {ov['subcategory']}")
    if ov["also_slot"] not in slot_ids: errors.append(f"slot_override unknown slot {ov['also_slot']}")
...
for p in r["penalties"]:
    if p["rule"] == "more_than_two_bold_patterns" and "note" in p:
        ...
        if any(f"= {x}" in note_text or f"={x}" in note_text for x in ["graphic_print", "floral"]):
            errors.append("penalty note hand-lists bold patterns again — must defer to patterns[].bold")

# ATR mapping targets real slots
for k, v in t["atr_to_slot"].items():
    if v is not None and v not in slot_ids: errors.append(f"atr_to_slot: {k} -> unknown slot {v}")
...
for a, lst in comp.items():
    for b in lst:
        if b not in dc_ids: errors.append(f"compat: unknown dress_code {b}")
        elif a not in comp.get(b, []): warnings.append(f"compat asymmetric: {a}->{b} but not {b}->{a}")

# formality ranges within 1..5
for d in t["dress_codes"]:
    lo, hi = d["formality_range"]
    if not (1 <= lo <= hi <= 5): errors.append(f"{d['id']} bad formality_range")
...
for o in t["occasions"]:
    if o["dress_code"] not in dc_ids:
        ...
        if not lo <= o["formality_target"] <= hi:
            errors.append(f"occasion {o['id']}: formality {o['formality_target']} outside {o['dress_code']} range {lo}-{hi}")

# colours: unique ids, hex present except multi
seen = set()
for c in t["colours"]:
    if c["id"] in seen: errors.append(f"duplicate colour {c['id']}")
    seen.add(c["id"])
    if c["family"] != "multi" and not c.get("hex"): errors.append(f"colour {c['id']} missing hex")

# fields reference known tiers; eval_floors match field names
for f, cfg in t["fields"].items():
    if cfg["tier"] not in {"local", "vlm", "rule"}: errors.append(f"field {f} bad tier")
    if cfg["tier"] == "rule" and cfg["source"] == "derived" and f == "climate_bands" and "climate_band_rules" not in t:
        errors.append(f"field {f} is tier=rule/derived but no `climate_band_rules` table exists")
for f in t["eval_floors"]:
    if f != "by_slice" and f not in t["fields"]: errors.append(f"eval_floor for unknown field {f}")

# eval_floors.by_slice must cover every slice GOLDEN_SET_SPEC.md quotas
GOLDEN_SLICES = {"flat_lay", "on_hanger", "worn_single", "worn_multi",
                  "ethnic_wear", "dark_on_dark", "pattern_heavy", "folded",
                  "low_light"}
...
if by_slice - GOLDEN_SLICES:
    warnings.append(f"eval_floors.by_slice has slices not in GOLDEN_SET_SPEC.md: {sorted(by_slice - GOLDEN_SLICES)}")
...
def band_matches(b, temp, hum, precip):
    ...
    if lo is not None and temp < lo: return False
    if hi is not None and temp >= hi: return False
    if b.get("precip") and not precip: return False
    if "humidity_pct" in b:
        h1, h2 = b["humidity_pct"]
        if not (h1 <= hum <= h2): return False
    return True
...
if gaps:
    errors.append(f"climate_bands: {len(gaps)} (temp,humidity,precip) combinations match NO band, e.g. {gaps[:3]}")

# fields marked required with no producer rule
for f, c in t["fields"].items():
    if c["required"] and c["tier"] == "rule" and c["source"] == "derived" and "climate_band_rules" not in t:
        errors.append(f"field {f}: required + derived but no derivation table in taxonomy.yaml")

# no 'other' anywhere
if any("other" == s for s in flat) or "other" in t["materials"]:
...
print(f"patterns             {len(t['patterns'])}  ({sum(1 for p in t['patterns'] if p['bold'])} bold)")
print(f"fits                 {len(t['fits'])}")
print(f"dress_codes          {len(dc_ids)}")
print(f"occasions            {len(t['occasions'])}")
print(f"fields               {len(t['fields'])}  ({sum(1 for c in t['fields'].values() if c['tier']=='vlm')} need the VLM)")
print()
